...
from .robotdata import RobotPose, getPose, DefineJointPose
...
class RobotMotion:
    def __init__(self, baseLink: str, linkNames: List[str], armType: ArmType = ArmType.KHI_RS007N):
        self.animating = True
        self.sequence = [
            RobotPose.zero, RobotPose.test, RobotPose.deg10, RobotPose.rest, RobotPose.fcartup, RobotPose.fcartdn,
    ...
    def next_pose(self,array):
        next = DefineJointPose(RobotPose.next,array)
        self.pose_sequence.put(next)



    def move(self):
        self.lastPose = self.pose if self.pose is not None else self.lastPose
        if self.pose_sequence.not_empty():
            self.pose = self.pose_sequence.get()
